...
class ILMPCAttentionPolicy(tf.keras.Model):
    def __init__(self, hidden=HIDDEN_NEURONS):
        super().__init__()
        self.state_enc = tf.keras.Sequential([
            tf.keras.layers.Dense(hidden, activation='relu'),
            tf.keras.layers.Dense(hidden, activation='relu'),  # deeper
        ], name='state_encoder')

        self.wp_enc = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(hidden, activation='relu'),
            name='waypoint_encoder'
        )
        #self.gate_proj = tf.keras.layers.Dense(hidden, name='gate_proj')
    ...
    def call(self, inputs, return_weights=False):
        ...
        state_embed = tf.expand_dims(self.state_enc(query), 1)  # (B, 1, hidden)
        wp_embed = self.wp_enc(waypoints)  # (B, 2, hidden)
        pooled = tf.reduce_mean(tf.concat([state_embed, wp_embed], axis=1), axis = 1)

        # gate = tf.nn.sigmoid(self.gate_proj(goalswitch))  # (B, 64)
        # query = state_embed * gate
    ...
    #     V = self.W_v(kv)  # (B, 2, hidden)
        ...
        # Output
        out = self.out_act(self.fc2(self.fc1(pooled)))  # (B, 3)
...
@tf.function
def train_step(model, optimizer, Xq, Xw, Y):
    with tf.GradientTape() as tape:
        pred = model([Xq, Xw], return_weights=False)
...
def train(model, Xq_tr, Xw_tr, Y_tr, Yat_tr,
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0, len(idx), BATCH_SIZE):
            ...
            lt = train_step(
                model, optimizer,
                Xq_tr[b], Xw_tr[b], Y_tr[b]
            )
            t_lst.append(float(lt))

        # Validation
        vp = model([Xq_val, Xw_val], return_weights=False)
        vp = vp.numpy()
...
def evaluate(model, Xq_val, Xw_val, Y_val, gs_val):
    pred = model([Xq_val, Xw_val], return_weights=False)
    pred = pred.numpy()
...
def plot_training(history):
    ...
    ax[0].plot(history['loss_track'])
    ax[0].set_title('Tracking loss (weighted MAE)')
    ax[0].set_xlabel('Epoch')

    ax[1].plot(history['val_mae'], color='steelblue')
    ax[1].set_title('Validation weighted MAE')
    ax[1].set_xlabel('Epoch')

    plt.tight_layout()
    plt.savefig('training_curves_BRIDGEONLY.pdf', bbox_inches='tight', dpi=150)
    plt.close()
...
def main():
    ...
    print(f"  Bridge model     val_mae = {hist['val_mae'][-1]:.4f}")
