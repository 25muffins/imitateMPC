...
class SigmoidBlend(tf.keras.Model):
...
def train(Xq_tr, Xw_tr, Xh_tr, gs_tr, Y_tr,  #train
    ...
    def step(Xq, Xw, Xh, gs, Y):
        with tf.GradientTape() as tape:
            p, sigmoid_w = network([Xq, Xw, Xh, gs])
            err = tf.abs(p - Y)
    ...
    vp, sigmoid_w = network([Xq_val, Xw_val, Xh_val, gs_val])
    network.summary()
    vp = vp.numpy()
    err = np.abs(vp - Y_val)
    ...
    print(sigmoid_w.numpy().mean(axis = 0))
...
def main():
    ...
    print(f"\nAblation table:")
    print(f"  sigmoid_blend        val_mae = {mae:.4f}  |  val_mse = {mse:.4f}")
