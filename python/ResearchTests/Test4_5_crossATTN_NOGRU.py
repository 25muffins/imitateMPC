...
SAVE_PATH = 'tfliteFiles/Test4_crossATTN_NOGRU.tflite'
...
class crossAttn(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        inputFeatures = tf.gather(query, [0, 1, 2, 3, 7], axis=1)
        ...
        #summarize
        # H_enriched = tf.concat([history, context], axis=-1)  # (B, 8, 128)
        ...
        combined = tf.concat([inputFeatures, context, wp_flat], axis=-1)  # (B, 192)
...
def main():
    ...
    (Xq_tr, Xq_val, Xw_tr, Xw_val,
     Y_tr, Y_val, gs_tr, gs_val, Xh_tr, Xh_val, Yat_tr) = temporal_split(
