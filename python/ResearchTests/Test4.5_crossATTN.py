...
SAVE_PATH = 'tfliteFiles/Test67_crossATTN.tflite'
...
class crossAttn(tf.keras.Model):
    def __init__(self, hidden=HIDDEN_NEURONS):
        ...
        self.wp_enc2 = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(hidden, activation='relu'),
            name='wp_enc'
        )
    ...
    def call(self, inputs):
        ...
        # h_gru = self.history_gru(history)  # (B, 64)
        #wp blend

        #just concat all together
        combined = tf.concat([input_enc, context, wp_flat], axis=-1)  # (B, 192)
